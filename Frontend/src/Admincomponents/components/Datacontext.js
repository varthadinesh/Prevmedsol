...
export function DataProvider({ children }) {
  ...
  const [doctor, setDoctor] = useState(() => {
    ...
    const [labagent, setLabagent] = useState(() => {
      const storedLabagent = JSON.parse(localStorage.getItem('labagent'));
        return storedLabagent || { firstname: '',lastname:'', email:'' };
      });
  ...
  return (
    <DataContext.Provider value={{ data, addToData, cartItems, addToCart, removeFromCart, user, setUserData, doctor, setDoctorData, labagent, setLabagentData}}>
      {children}
