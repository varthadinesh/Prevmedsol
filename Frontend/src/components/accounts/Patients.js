...
const Patients = () => {
  ...
  const handleSave = (e) => {
    e.preventDefault();
    axios.post(`http://localhost:8080/patients`, editedData)
      .then((res) => {
        console.log(res);
  ...
  const handleDelete = (index) => {
    ...
    axios.delete(`http://localhost:8080/patients`, { data: { email: emailToDelete } })
        .then((res) => {
            console.log(res);
            window.location.reload(false);
  ...
  return (
    <>
    {isEditing ? (<Editpatient replaceEdit={replaceEdit} editedData={editedData} handleChange={handleChange} handleSave={handleSave}></Editpatient>) :
